...
#include <signalLeftMode.h>
#include <signalRightMode.h>
...
boost::shared_ptr<Mode> ModeFactory::create(cob_light::LightMode requestMode, IColorO* colorO)
{
  ...
  switch (requestMode.mode)
  {
    case cob_light::LightModes::STATIC:
      mode.reset(
          new StaticMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses, requestMode.timeout));
      break;

    case cob_light::LightModes::FLASH:
      mode.reset(
          new FlashMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses, requestMode.timeout));
      break;

    case cob_light::LightModes::BREATH:
      mode.reset(
          new BreathMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses, requestMode.timeout));
      break;

    case cob_light::LightModes::BREATH_COLOR:
      mode.reset(new BreathColorMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses,
                                     requestMode.timeout));
      break;

    case cob_light::LightModes::FADE_COLOR:
      mode.reset(new FadeColorMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses,
                                   requestMode.timeout));
      break;

    case cob_light::LightModes::SIGNAL_LEFT:
      mode.reset(new SignalLeft(color, colorO->getNumLeds(), requestMode.priority, requestMode.frequency,
                                requestMode.pulses, requestMode.timeout));
      break;

    case cob_light::LightModes::SIGNAL_RIGHT:
      mode.reset(new SignalRight(color, colorO->getNumLeds(), requestMode.priority, requestMode.frequency,
                                requestMode.pulses, requestMode.timeout));
      break;
    ...
    case cob_light::LightModes::SEQ:
    {
      ...
      mode.reset(
          new SequenceMode(seqs, requestMode.priority, requestMode.frequency, requestMode.pulses, requestMode.timeout));
    }
    break;
    ...
    case cob_light::LightModes::GLOW:
      mode.reset(new GlowColorMode(color, requestMode.priority, requestMode.frequency, requestMode.pulses,
                                   requestMode.timeout));
      break;

    case cob_light::LightModes::XMAS:
      mode.reset(new XMasMode(colorO->getNumLeds(), requestMode.priority, requestMode.frequency, requestMode.pulses,
                              requestMode.timeout));
      break;
