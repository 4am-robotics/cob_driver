         ...
         kinematics_msgs::GetPositionIK::Response &res )
{
	...
	if(ret < 0)
	{
		res.error_code.val = motion_planning_msgs::ArmNavigationErrorCodes::PLANNING_FAILED;
	...
	else
	{
		...
		res.error_code.val = motion_planning_msgs::ArmNavigationErrorCodes::SUCCESS;
		for(int i = 0; i < nj; i++)	
